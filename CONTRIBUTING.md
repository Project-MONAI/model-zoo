...
In inference config file (if exists), please include the following keys: `bundle_root` (root directory of your bundle), `device` (required device), `network_def` (definition of the network components), `output_dir` (directory to contain inference results) and `inferer` ([click here for instance](https://github.com/Project-MONAI/model-zoo/blob/dev/models/brats_mri_segmentation/configs/inference.json)).
...
1. Please include keys `bundle_root`, `device` and `dataset_dir`, and if there are any output files, the directory should also be defined as `output_dir`.
