...
Check the functionality of exporting the checkpoint to [TensorRT](https://developer.nvidia.com/tensorrt) based models. TensorRT based models target NVIDIA GPUs via NVIDIA’s TensorRT Deep Learning Optimizer and Runtime. It can accelerate models' inference on NVIDIA GPU with speedup ratio up to 6x by converting models weight to float32 or float16 precision. In MONAI, models are compiled to TensorRT based models through [Torch_TensorRT](https://pytorch.org/TensorRT/). To use the TensorRT conversion to accelerate model inference, you must have a NVIDIA GPU, install TensorRT and Torch-TensorRT. We have started testing most of our models on NVIDIA A100 80G GPU with **Torch_TensorRT version >= 1.4.0** and **TensorRT version >= 8.5.3**. Please make sure your environment meets this minimum version requirement. Or you can use the MONAI Docker where the **MONAI version is >= 1.2**. Currently only a part of models in MONAI model-zoo support TensorRT conversion. More models will be covered in the future with new Torch-TensorRT and TensorRT version. For the specific version of these two libraries that can support TensorRT conversion for corresponding models, please go to the README file in the bundle. You can also run the following command locally to export your bundle in float32 precision or float16 precision.
