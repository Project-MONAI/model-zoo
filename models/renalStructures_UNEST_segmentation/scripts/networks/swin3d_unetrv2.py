...
from monai.networks.blocks.unetr_block import UnetrBasicBlock, UnetrPrUpBlock, UnetResBlock
...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        print('Before swinViT: {}'.format(x_in.shape))
        hidden_states_out = self.swinViT(x_in)
        print('After swinViT 0: {}'.format(hidden_states_out[0].shape))
        print('After swinViT 1: {}'.format(hidden_states_out[1].shape))
        print('After swinViT 2: {}'.format(hidden_states_out[2].shape))
        print('After swinViT 3: {}'.format(hidden_states_out[3].shape))
        print('After swinViT 4: {}'.format(hidden_states_out[4].shape))

        enc0 = self.encoder1(x_in)

        print('enc0: {}'.format(enc0.shape))
        ...
        print('dec4: {}'.format(dec4.shape))

        dec4 = self.encoder10(dec4)
        print('dec4 after encoder10: {}'.format(dec4.shape))

        dec3 = self.decoder5(dec4, enc4)
        print('dec3 after encoder10: {}'.format(dec3.shape))

        dec2 = self.decoder4(dec3, enc3)
        print('dec2 after encoder10: {}'.format(dec2.shape))

        dec1 = self.decoder3(dec2, enc2)
        print('dec1 after encoder10: {}'.format(dec1.shape))

        dec0 = self.decoder2(dec1, enc1)
        print('dec0 after encoder10: {}'.format(dec0.shape))

        out = self.decoder1(dec0, enc0)
        print('out after encoder10: {}'.format(out.shape))
