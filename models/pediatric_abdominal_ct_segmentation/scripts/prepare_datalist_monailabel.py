    ...
    if "train" in splits:
        ...
        if "valid" in splits:
            val_list, test_list = train_test_split(
                other_list, train_size=valid_test_split
            )
            return {
                "training": train_list,
                "validation": val_list,
...
def keep_image_label_pairs_only(a_images, a_labels, i_folder, l_folder):
    ...
    a_images = sorted(
        [os.path.join(i_folder, a) for a in image_names if a in label_names]
    )
    ## Keep only labels that have a scan
    image_names = [a.split("/")[-1] for a in a_images]
    a_labels = sorted(
        [os.path.join(l_folder, a) for a in label_names if a in image_names]
    )
...
def parse_files(images_folder, labels_folder, file_extension_pattern):
    logging.info(
        f"parsing files at: {os.path.join(images_folder, file_extension_pattern)}"
    )
...
def get_datalist(args, images_folder, labels_folder):
    ...
    if type(images_folder) is list:
        ...
        for ifolder, lfolder in zip(images_folder, labels_folder):
            ...
            a_images, a_labels = keep_image_label_pairs_only(
                a_images, a_labels, ifolder, lfolder
            )
            all_images += a_images
            all_labels += a_labels
    else:
        all_images, all_labels = parse_files(
            images_folder, labels_folder, file_extension_pattern
        )
        all_images, all_labels = keep_image_label_pairs_only(
            all_images, all_labels, images_folder, labels_folder
        )
...
def main(args):
    ...
    datalist = get_datalist(
        args, data_file_base_dir, os.path.join(data_file_base_dir, args.labels_folder)
    )
    datalist = produce_datalist_splits(
        datalist, args.splits, args.train_split, args.valid_test_split
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_split", type=int, default=0.80, help="fraction of Training samples."
    )
    parser.add_argument(
        "--valid_test_split",
        type=int,
        default=0.50,
        help="fraction of valid/test samples.",
    )
    parser.add_argument(
        "--splits",
        type=list,
        default=["test"],
        help="splits to use for train, valid, and test.",
    )
    parser.add_argument(
        "--file_extension",
        type=str,
        default="nii",
        help="file extension of images and labels.",
    )
    parser.add_argument(
        "--labels_folder",
        type=str,
        default="labels/final",
        help="labels sub folder name",
    )
