    ...
    **kwargs: Any,
    ...
    if any(pad_size):
        ...
        if "labels" in kwargs.keys() and kwargs["labels"] is not None:
            kwargs["labels"] = F.pad(
                kwargs["labels"], pad=pad_size, mode=look_up_option(padding_mode, PytorchPadMode), value=cval
            )
        if "prev_mask" in kwargs.keys() and kwargs["prev_mask"] is not None:
            kwargs["prev_mask"] = F.pad(
                kwargs["prev_mask"], pad=pad_size, mode=look_up_option(padding_mode, PytorchPadMode), value=cval
            )
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        kwargs["patch_coords"] = unravel_slice[0]
