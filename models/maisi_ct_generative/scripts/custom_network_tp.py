...
class SplitConvolutionV1(nn.Module):
    ...
    def forward(self, x):
        ...
        if max(outputs[0].size()) < 500:
            ...
            x = torch.cat(list(outputs), dim=self.tp_dim + 2)
        else:
            import gc
...
class SplitResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = InplaceGroupNorm3D(num_groups=norm_num_groups, num_channels=in_channels, eps=norm_eps, affine=True)
        ...
        self.norm2 = InplaceGroupNorm3D(
            num_groups=norm_num_groups, num_channels=out_channels, eps=norm_eps, affine=True
        )
...
class EncoderTp(nn.Module):
    ...
    def __init__(
        ...
        blocks.append(
            InplaceGroupNorm3D(num_groups=norm_num_groups, num_channels=num_channels[-1], eps=norm_eps, affine=True)
        )
        blocks.append(
            SplitConvolutionV1(
        ...
        for _i in range(len(self.blocks)):
            ...
            # if not isinstance(block, monai.networks.blocks.convolutions.Convolution):
                ...
                if False:
                    ...
                    if self.tp_dim == 0:
                        splits = [
                            x[:, :, i * split_size - overlaps[i] : (i+1) * split_size +
...
class DecoderTp1(nn.Module):
    ...
    def __init__(
        ...
        blocks.append(
            InplaceGroupNorm3D(num_groups=norm_num_groups, num_channels=block_in_ch, eps=norm_eps, affine=True)
        )
        blocks.append(
            SplitConvolutionV1(
                spatial_dims=spatial_dims,
...
class AutoencoderKlckModifiedTp(AutoencoderKL):
    """
